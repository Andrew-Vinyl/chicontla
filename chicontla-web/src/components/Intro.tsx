...
export default function Intro({ data }: IntroProps) {
  // Read from the new intro-prefixed homepage fields
  const eyebrow = data?.introEyebrow ?? FALLBACK.eyebrow;
  const headingBlack = data?.introHeadingBlack ?? FALLBACK.headingBlack;
  const headingGreen = data?.introHeadingGreen ?? FALLBACK.headingGreen;
  const pullQuote = data?.introPullQuote ?? FALLBACK.pullQuote;
  const photoCaption = data?.introPhotoCaption ?? FALLBACK.photoCaption;
  const photoSubcaption = data?.introPhotoSubcaption ?? FALLBACK.photoSubcaption;
  const statValue = data?.introStatValue ?? FALLBACK.statValue;
    ...
    <section className="bg-surface py-24 lg:py-40 border-b border-background/10 overflow-hidden grain-overlay">
      <div className="container mx-auto px-6 max-w-[1320px] relative z-10">
        ...
        >
          ...
          >
            {headingBlack}{" "}
            <span className="text-primary">{headingGreen}</span>
          </motion.h2>
        </motion.div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16 items-start">
          ...
          >
            ...
            <div className="pl-6 py-5 border-l-4 border-donate relative bg-background/5 rounded-r-2xl pr-6 flex items-center gap-6 transition-all duration-300 hover:-translate-y-1 hover:shadow-md cursor-default">
              <p className="font-heading font-black text-5xl text-donate shrink-0">{statValue}</p>
